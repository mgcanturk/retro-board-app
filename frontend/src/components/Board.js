...
const Board = () => {
  ...
  const [maskedNames, setMaskedNames] = useState({});
  ...
  const initializeSocket = (userNickname, userIsAdmin, userInviteCode) => {
    ...
    socketRef.current.on('boardState', (boardState) => {
      ...
      // Masked names haritası oluştur
      if (boardState.participants) {
        const map = {};
        boardState.participants.forEach((p, idx) => {
          map[p.nickname] = String.fromCharCode(65 + idx) + '***';
        });
        setMaskedNames(map);
      }
    ...
    socketRef.current.on('userJoined', ({ nickname, isAdmin, participantCount }) => {
      setParticipantCount(participantCount);
    });
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 items-center justify-end w-full md:space-x-4 md:flex-nowrap md:gap-0">
              {/* Participant Count */}
              <div className="flex items-center text-gray-600 relative">
                ...
                {showParticipants && (
                  <div className="absolute left-0 top-7 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg p-2 min-w-[180px]">
                    {board?.participants?.map((p, idx) => (
                      <span key={p.nickname} className={`block mb-1 px-2 py-0.5 rounded text-xs font-semibold ${p.isAdmin ? 'bg-gradient-to-r from-green-200 to-emerald-300 dark:from-emerald-700 dark:to-green-900 text-black dark:text-white border border-green-400 dark:border-green-700 shadow' : (p.nickname === currentNickname ? 'bg-blue-200 dark:bg-blue-700 text-blue-800 dark:text-blue-100' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100')}`}>{p.nickname}{p.isAdmin ? ' (Admin)' : ''}{p.nickname === currentNickname ? ' (Sen)' : ''}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              {isAdmin && (
                <button
                  ...
                  className={`flex items-center px-2 py-1 rounded-md text-xs md:text-sm md:px-3 md:py-1 transition-all duration-200 ${
                    showNames 
                ...
                >
                  ...
                  {showNames ? 'İsimler Görünür' : 'İsimler Gizli'}
                </button>
              )}
              ...
              {isAdmin && (
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(!isTimerActive || timeLeft === 0) ? (
                    <button
                      onClick={startTimer}
                      className="flex items-center px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-xs md:text-sm md:px-3 md:py-1"
                      disabled={false}
                    >
                    ...
                    <button
                      onClick={stopTimer}
                      className="flex items-center px-2 py-1 bg-red-100 text-red-700 rounded-md text-xs md:text-sm md:px-3 md:py-1"
                    >
              ...
              {isAdmin && (
                <button
                  ...
                  className={`flex items-center px-2 py-1 bg-gray-100 text-gray-700 rounded-md text-xs md:text-sm md:px-3 md:py-1 hover:bg-gray-200 ${darkMode ? 'dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600' : ''}`}
                >
              ...
              {isAdmin && (
                <button
                  onClick={exportBoard}
                  className="flex items-center px-2 py-1 bg-green-100 text-green-700 rounded-md text-xs md:text-sm md:px-3 md:py-1 hover:bg-green-200"
                >
              ...
              {isAdmin && (
                <button
                  onClick={endBoard}
                  className="flex items-center px-2 py-1 bg-red-600 text-white rounded-md text-xs md:text-sm md:px-3 md:py-1 hover:bg-red-700"
                >
                  Boardı Sonlandır
                </button>
              )}
              ...
              <button
                ...
                className={`flex items-center justify-center w-8 h-8 md:w-10 md:h-8 rounded-md text-xs md:text-sm transition-colors ${darkMode ? 'bg-gray-800 text-white hover:bg-gray-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                ...
                {column.comments && column.comments.map((comment, idx) => (
                  <div key={comment.id} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 relative border border-gray-200 dark:border-gray-600">
                    ...
                    <p className="text-gray-800 dark:text-gray-100 text-sm mb-2">
                      {isLocked ? comment.text : '****'}
                    </p>
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                      <span>
                        {showNames
                          ? comment.author
                          : (comment.author ? (comment.author[0]?.toLowerCase() + '*') : '?')}
                      </span>
                      <span>
